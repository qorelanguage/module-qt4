...
void ClassMap::registerSmoke(Smoke * smoke)
{
     // add class hierarchy information
     setupClassHierarchy(smoke);

     QoreClass *qc = 0;
     QByteArray mname;
     Q_ASSERT_X(smoke->numMethodMaps > 1, "numMethodMaps check", "empty smoke propably");
     for (int i = 1; i < smoke->numMethodMaps; ++i) {
          Smoke::MethodMap &mm = smoke->methodMaps[i];
          Smoke::Class &c = smoke->classes[mm.classId];
          ...
          QByteArray munged = smoke->methodNames[mm.name];

          // overloads are < 0. Done in ambiguousMethodList part of this condition
          if (mm.method > 0) {
               const Smoke::Method &m = smoke->methods[mm.method];
               mname = smoke->methodNames[m.name];
               ...
               while (smoke->ambiguousMethodList[ambIx]) {
                    Smoke::Index ambMethIx = smoke->ambiguousMethodList[ambIx];
                    const Smoke::Method &m = smoke->methods[ambMethIx];
                    mname = smoke->methodNames[m.name];
...
void ClassMap::addQoreMethods()
{
     ...
     qc->addMethodExtended("scanLine", (q_method_t)QIMAGE_scanLine, false, QC_NO_FLAGS, QDOM_GUI, binaryTypeInfo, 1, bigIntTypeInfo, QORE_PARAM_NO_ARG);
...
void ClassMap::setupClassHierarchy(Smoke * smoke)
{
     for (Smoke::Index ix = 1; ix < smoke->numClasses; ++ix) {
          const Smoke::Class &c = smoke->classes[ix];
          QoreClass *qc = findCreateQoreClass(ix);
          for (Smoke::Index *i = smoke->inheritanceList + c.parents; *i; ++i) {
...
void ClassMap::addMethod(QoreClass *qc, const Smoke::Class &c, const Smoke::Method &method, const TypeHandler *th)
{
     ...
     if ((!strcmp(qc->getName(), "QAbstractItemModel") && !strcmp(methodName, "createIndex"))
               ...
               || (!strcmp(qc->getName(), "QPixmapCache") && !strcmp(methodName, "find"))
        ) {
     ...
     if ((method.flags & Smoke::mf_ctor)) {
          ...
          qc->setConstructorExtendedList3(th, (q_constructor3_t)common_constructor, isPrivate, QC_NO_FLAGS, QDOM_GUI, argTypeInfo);
          return;
     }
     ...
     if (method.flags & Smoke::mf_static) {
          ...
          qc->addStaticMethodExtendedList3(th, methodName, func, isPrivate, QC_NO_FLAGS, QDOM_GUI, returnTypeInfo, argTypeInfo);
          return;
     }
     ...
     qc->addMethodExtendedList3(th, name, func, isPrivate, QC_NO_FLAGS, QDOM_GUI, returnTypeInfo, argTypeInfo);
}

void ClassMap::registerMethod(const char *class_name, const char *method_name, const char *munged_name, Smoke::Index method_index, ClassMap::TypeHandler &type_handler, const QoreTypeInfo *returnType, const type_vec_t &argTypeList, bool is_static)
{
//     assert(m_map[class_name][method_name][munged_name].types.count() == 0);
     ...
     if (!strcmp(class_name, method_name)) {
          ...
          qc->setConstructorExtendedList3(th, (q_constructor3_t)common_constructor, false, QC_NO_FLAGS, QDOM_GUI, argTypeList);
     } else {
          //printd(5, "ClassMap::registerMethod() registering function %s::%s() args=%d th=%p\n", class_name, method_name, argTypeList.size(), th);
          if (is_static)
               qc->addStaticMethodExtendedList3(th, method_name, (q_static_method3_t)common_static_method, false, QC_NO_FLAGS, QDOM_GUI, returnType, argTypeList);
          else
               qc->addMethodExtendedList3(th, method_name, (q_method3_t)common_method, false, QC_NO_FLAGS, QDOM_GUI, returnType, argTypeList);
     }
...
{
     ClassMap::TypeHandler ret(qt_Smoke);
...
//         printd(0, "common_constructor() %s failed to set up constructor call\n", className);
     ...
     if (myclass.getClass(QC_QOBJECT->getID())) {
          ...
          if (myclass.getClass(QC_QABSTRACTITEMMODEL->getID()))
               obj = new QoreSmokePrivateQAbstractItemModelData(cqm.smoke(), cqm.method().classId, qobj, self);
          else
               obj = new QoreSmokePrivateQObjectData(cqm.smoke(), cqm.method().classId, qobj, self);

          is_qobject = true;
     } else {
          obj = new QoreSmokePrivateData(cqm.smoke(), cqm.method().classId, qtObj, self);
...
void common_destructor(const QoreClass &thisclass, ClassMap::TypeHandler *type_handler, QoreObject *self, AbstractPrivateData *private_data, ExceptionSink *xsink)
{
     ...
     const char * className = p->getClassName();
...
QoreSmokePrivateData::~QoreSmokePrivateData()
{
     ...
     if (m_object && !externallyOwned()) {
          const Smoke::Class &cls = m_smoke->classes[classIndex()];
