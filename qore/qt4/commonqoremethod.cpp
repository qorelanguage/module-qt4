       ...
       args (0)
{
     ...
     m_method = m_smoke->methods[type_handler->method];
     //printd(0, "CommonQoreMethod::method() method=%d (%s)\n", type_handler.method, m_smoke->methodNames[m_method.name]);
       ...
       args(params)
{
     ...
     //printd(0, "CommonQoreMethod::method() method=%d smoke: %s::%s()\n", type_handler->method, m_smoke->classes[m_method.classId].className, m_smoke->methodNames[m_method.name]);
          ...
          for (RefMap::iterator i = temp_store->begin(), e = temp_store->end(); i != e; ++i) {
               ...
               if (rf.ref) {
                    ...
                    case temp_store_s::r_qpixmap:
                         ref.assign(Marshalling::createQoreObjectFromNonQObject(m_smoke, QC_QPIXMAP, SCI_QPIXMAP, rf.data.q_qpixmap), m_xsink);
                         rf.data.q_qpixmap = 0;
                         break;
                    default:
...
int CommonQoreMethod::returnQtObjectOnStack(Smoke * smoke, Smoke::StackItem &si, const char *cname, const char *mname, const AbstractQoreNode *v, Smoke::Type &t, int index, ExceptionSink *xsink, bool temp, temp_store_s *temp_store)
{
     ...
     if (p) {
          ...
          if (temp && flags == Smoke::tf_stack) {
               if (v->is_unique())
                    ...
                    p = Marshalling::constructCopy(smoke, p, smoke->classes[t.classId].className, xsink);
                    if (*xsink)
                         return -1;
                    if (temp_store)
                         temp_store->assign(smoke, t.classId, p);
               }
               ...
               if (t.flags & Smoke::tf_const) {
                  ...
                  p = Marshalling::constructCopy(p, m_smoke->classes[t.classId].className, xsink);
                  if (*xsink)
                return -1;
               }
               */
...
int CommonQoreMethod::qoreToStackStatic(Smoke * smoke,
                                        ExceptionSink *xsink,
                                        Smoke::StackItem &si,
                                        const char *className,
                                        const char *methodName,
...
//                 printd(0, "CommonQoreMethod::qoreToStack() processing list %p size %d\n", l, l->size());
               ...
               */
               ...
               return returnQtObjectOnStack(smoke, si, className, methodName, v, t, index, xsink, temp, temp_store);
...
//                 printd(0, "CommonQoreMethod::qoreToStackStatic handling a list argument %s\n", name);
                    Marshalling::QoreQListBase * list = Marshalling::QoreToQtContainer::Instance()->marshall(smoke, t, node, xsink);
                    if (!list) {
                    ...
                    std::auto_ptr<Marshalling::QoreQVariant> variant(Marshalling::qoreToQVariant(smoke, t, node, xsink));
...
//             printd(0, "can't handle ref type '%s'\n", t.name);
     ...
     if (t.name[0] == 'Q')
          return returnQtObjectOnStack(smoke, si, className, methodName, node, t, index, xsink, temp, temp_store);
...
int CommonQoreMethod::qoreToStack(Smoke::Type t,
                                  ...
                                  int index)
{
     return qoreToStackStatic(m_smoke, m_xsink, Stack[index], m_className, m_methodName, t, node, index, this);
}
...
void *CommonQoreMethod::callConstructor()
{
     ...
     Smoke::StackItem a[2];
     a[1].s_voidp = QoreSmokeBinding::Instance(m_smoke);
