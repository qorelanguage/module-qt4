...
public:
     DLLLOCAL QoreSmokePrivate(Smoke * s, Smoke::Index classID, bool n_is_qobject = false)
        : m_smoke(s), m_class(classID), externally_owned(false), is_qobject(n_is_qobject), obj_ref(false) {}
     ...
     DLLLOCAL const char *getClassName() const {
          return m_smoke->classes[m_class].className;
     }
...
class QoreSmokePrivateData : public QoreSmokePrivate {
     bool save_map;
public:
     DLLLOCAL QoreSmokePrivateData(Smoke * s, Smoke::Index classID, void *p, QoreObject *self)
        : QoreSmokePrivate(s, classID), m_object(p) {
          // register in object map if object has virtual functions
          save_map = m_smoke->classes[classID].flags & Smoke::cf_virtual;
          if (save_map)
...
public:
     DLLLOCAL QoreSmokePrivateQObjectData(Smoke * s, Smoke::Index classID, QObject *p, QoreObject *self)
        : QoreSmokePrivate(s, classID, true), m_qobject(p) {
          ...
          Smoke::ModuleIndex mi = m_smoke->findMethod(m_smoke->classes[classID].className, "qt_metacall$$?");
          assert(mi.smoke);
          qt_metacall_index = m_smoke->methodMaps[mi.index].method;
          assert(qt_metacall_index > 0);
          assert(!strcmp(m_smoke->methodNames[m_smoke->methods[qt_metacall_index].name], "qt_metacall"));

          if (p->isWidgetType()) {
               assert(strcmp(m_smoke->classes[classID].className, "QApplication"));
     ...
     DLLLOCAL int createSignal(const char *signal, ExceptionSink *xsink) {
          ...
          std::auto_ptr<QoreQtDynamicSignal> ds(new QoreQtDynamicSignal(m_smoke, signal, xsink));
          if (*xsink)
               return -1;
     ...
     DLLLOCAL void emitSignal(const char *sig, const QoreListNode *args, ExceptionSink *xsink) {
          ...
          if (id >= 0) {
               ...
               emitStaticSignal(m_smoke, m_qobject.data(), id, qmm, args, xsink);
...
protected:
     ...
     DLLLOCAL int getSlotIndex(const QoreObject *receiver, const QByteArray &theSlot, const char *sig, ExceptionSink *xsink) {
          ...
          if (!special_method && meth) {
               std::auto_ptr<QoreQtDynamicSlot> ds(new QoreQtDynamicSlot(m_smoke, receiver, meth, sig, xsink));
               if (*xsink)
                    return -1;
...
public:
     DLLLOCAL QoreSmokePrivateQAbstractItemModelData(Smoke * s, Smoke::Index classID, QObject *p, QoreObject *self)
        : QoreSmokePrivateQObjectData(s, classID, p, self) {
     }
...
public:
     ...
     typedef struct type_handler_s {
          ...
          Smoke::Index method;
          DLLLOCAL type_handler_s(Smoke * s, const TypeList &n_types, arg_handler_t n_handler, return_value_handler_t n_rv_handler, Smoke::Index n_method) :
               smoke(s), types(n_types), arg_handler(n_handler), return_value_handler(n_rv_handler), method(n_method) {}
          DLLLOCAL type_handler_s(Smoke * s)
                : smoke(s), arg_handler(0), return_value_handler(0), method(0) {}
     } TypeHandler;
...
//     }
     ...
     DLLLOCAL static void init(Smoke * smoke) {
          if (!m_instance)
              new ClassMap();
          m_instance->registerSmoke(smoke);
     }
     ...
     DLLLOCAL void registerMethod(const char *class_name, const char *method_name, const char *munged_name, Smoke::Index method_index, TypeHandler &type_handler, const QoreTypeInfo *returnType = 0, const type_vec_t &argTypeList = type_vec_t(), bool is_static = false);
...
private:
    ...
    DLLLOCAL void registerSmoke(Smoke * smoke);
     DLLLOCAL static void setupClassHierarchy(Smoke * smoke);
