...
#include "qorewriteinitialization.h"
#include "qorewriteiconinitialization.h"
#include "driver.h"
#include "ui4.h"
#include "utils.h"
#include "uic.h"
...
namespace {
    ...
    QString toolBarAreaStringFromDOMAttributes(const Qore::WriteInitialization::DomPropertyMap &attributes) {
...
void WriteInitialization::acceptUI(DomUI *node)
{
    ...
    if (m_option.autoConnection)
        m_output << "\n" << m_option.indent << "QMetaObject::connectSlotsByName($" << m_mainWidget << ");\n";
...
void WriteInitialization::acceptWidget(DomWidget *node)
{
	...
	parentWidget = toQoreParent(parentWidget, m_mainWidget);
    ...
    if (m_uic->isButton(className)) {
        ...
        if (const DomProperty *prop = attributes.value(QLatin1String("buttonGroup"))) {
            ...
            if (!m_buttonGroups.contains(groupName)) {
                ...
                m_output << m_option.indent << g << " = new QButtonGroup(" << m_generatedClass << ");\n";
            }
...
void WriteInitialization::acceptLayout(DomLayout *node)
{
    ...
    QString varName = toQoreIdentifier(m_driver->findOrInsertLayout(node));
    ...
    if (m_widgetChain.top()) {
        ...
        if (!m_layoutChain.top() && (m_uic->customWidgetsInfo()->extends(parentWidget, QLatin1String("Q3GroupBox"))
            ...
            const QString parent = toQoreIdentifier(m_driver->findOrInsertWidget(m_widgetChain.top()));
    ...
    if (!m_layoutChain.top() && !isGroupBox) {
        ...
        if (!parentWidget.isEmpty() && m_mainWidget == parentWidget)
...
void WriteInitialization::acceptLayoutItem(DomLayoutItem *node)
{
    ...
    const QString layoutName = toQoreIdentifier(m_driver->findOrInsertLayout(layout));
    const QString itemName = toQoreIdentifier(m_driver->findOrInsertLayoutItem(node));
...
{
    QString actionName = toQoreIdentifier(m_driver->findOrInsertActionGroup(node));
...
void WriteInitialization::acceptAction(DomAction *node)
{
    ...
    const QString actionName = toQoreIdentifier(m_driver->findOrInsertAction(node));

    m_registeredActions.insert(actionName, node);
    QString varName = m_driver->findOrInsertWidget(m_widgetChain.top());

    if (m_actionGroupChain.top()) {
        varName = toQoreIdentifier(m_driver->findOrInsertActionGroup(m_actionGroupChain.top()));
    }
...
void WriteInitialization::acceptActionRef(DomActionRef *node)
{
    ...
    QString rubyActionName = toQoreIdentifier(node->attributeName());
    bool isSeparator = actionName == QLatin1String("separator");
    bool isMenu = false;

    QString varName = toQoreIdentifier( m_driver->findOrInsertWidget(m_widgetChain.top()) );
    ...
    if (actionName.isEmpty() || !m_widgetChain.top()) {
        ...
        if (!isMenu && inQ3ToolBar) {
            m_actionOut << m_option.indent << rubyActionName << ".setParent(" << varName << ");\n";
            return;
        }
...
void WriteInitialization::writeProperties(const QString &varNameIn,
                                          const QString &className,
                                          const DomPropertyList &lst,
                                          unsigned flags)
{
	// HACK to handle QMainWidnow's self as parent - wrapping the setupUI(param) param to contain qore "$"
	QString varName(varNameIn);
    ...
    m_output << m_option.indent << indent << varName << ".setObjectName(" << fixString(varName, m_option.indent) << ");\n";
    ...
    for (int i=0; i<lst.size(); ++i) {
        ...
        if (stdset) {
            setFunction = QLatin1String(".")
                + "set" + propertyName.at(0).toUpper() + propertyName.mid(1)
                + QLatin1String("( ");
        } else {
            setFunction = QLatin1String(".setProperty(\"")
                + propertyName
                + QLatin1String("\", new QVariant(");
        }
        ...
        case DomProperty::Bool: {
            propertyValue = p->elementBool() == "true" ? "True" : "False";
            break;
        }
        ...
        case DomProperty::Palette: {
            ...
            m_output << m_option.indent << paletteName << " = new QPalette\n";
        ...
        case DomProperty::Locale: {
             ...
             propertyValue = QString::fromLatin1("new QLocale(QLocale::%1, QLocale::%2)")
...
QString WriteInitialization::writeFontProperties(const DomFont *f)
{
    ...
    m_output << m_option.indent << fontName << " = new QFont\n";
...
QString WriteInitialization::writeIconProperties(const DomResourceIcon *i)
{
    ...
    if (isIconFormat44(i)) {
        const QString pixmap = QLatin1String("new QPixmap");
        m_output << m_option.indent << iconName << " = new QIcon\n";
        ...
        m_output <<  m_option.indent << iconName << " = " << pixCall(QLatin1String("new QIcon.new"), i->text())<< "\n";
    }
    return iconName;
}
...
void WriteInitialization::writeBrush(const DomBrush *brush, const QString &brushName)
{
    ...
    if (style == QLatin1String("LinearGradientPattern") ||
        ...
        if (gradientType == QLatin1String("LinearGradient")) {
            m_output << m_option.indent << gradientName << " = QLinearGradient.new"
            ...
            m_output << m_option.indent << gradientName << " = QRadialGradient.new"
            ...
            m_output << m_option.indent << gradientName << " = QConicalGradient.new"
                << "(" << gradient->attributeCentralX()
                << ", " << gradient->attributeCentralY()
        ...
        m_output << m_option.indent << brushName << " = new QBrush" << "("
            << gradientName << ");\n";
...
{
    const QString varName = toQoreIdentifier(m_driver->findOrInsertWidget(w));
...
QString WriteInitialization::pixCall(const QString &t, const QString &text) const
{
    ...
    if (const DomImage *image = findImage(text)) {
        ...
        if (m_option.extractImages) {
            ...
            QString rc = QLatin1String("new QPixmap(\":/");
            rc += m_generatedClass;
...
{
	// TODO/FIXME: qore qt module does not support tr()s yet.
	//return '"' + str + '"';
    ...
    QString result;
    QString comment = commentHint.isEmpty() ? QString::fromUtf8("\"\"") : fixString(commentHint, m_option.indent);
    ...
    if (m_option.translateFunction.isEmpty()) {
        result += QLatin1String(", ");
        result += QLatin1String("QCoreApplication::UnicodeUTF8");
    }
...
{
    const QString menuName = toQoreIdentifier(m_driver->findOrInsertWidget(w));
...
//    sender = sender.mid(0, 1).toLower() + sender.mid(1);
    ...
    if (sender != m_mainWidget) {
        sender = toQoreIdentifier(sender);
    }
	else
		sender.prepend("$");
...
//    receiver = receiver.mid(0, 1).toLower() + receiver.mid(1);
    if (receiver != m_mainWidget) {
        receiver = toQoreIdentifier(receiver);
    }
	else
		receiver.prepend("$");
	...
	// TODO/FIXME: is it good qore syntax? Or should be changed?
#if 0
...
#endif
	m_output << m_option.indent
			<< receiver << ".connect(" << sender << ", SIGNAL('" << connection->elementSignal()
			<< "'), SLOT('" << connection->elementSlot() << "'));\n";
}
