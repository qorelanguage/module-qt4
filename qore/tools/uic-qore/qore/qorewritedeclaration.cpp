...
#include "qorewriteicondeclaration.h"
...
void WriteDeclaration::acceptUI(DomUI *node)
{
    ...
    if (m_option.generateNamespace && !m_option.prefix.isEmpty()) {
        ...
        while (it.hasNext()) {
            ...
            m_output << "namespace " << ns.mid(0, 1).toUpper() << ns.mid(1) << " {\n";
        }

        m_output << m_option.indent << "class "  << className << " inherits " << m_option.prefix << className << " {\n";
