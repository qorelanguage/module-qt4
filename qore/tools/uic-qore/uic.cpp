...
#ifdef QT_UIC_QORE_GENERATOR
#include "qorewritedeclaration.h"
#endif
...
#endif
    } else if (option().generator == Option::QoreGenerator) {
#ifdef QT_UIC_QORE_GENERATOR
        rtn = qorewrite(ui);
#else
        fprintf(stderr, "uic: option to generate qore code not compiled in\n");
#endif
    } else {
#ifdef QT_UIC_CPP_GENERATOR
...
// TODO/FIXME: merge rbwrite back from ruby SVN
#ifdef QT_UIC_QORE_GENERATOR
bool Uic::qorewrite(DomUI *ui)
{
    //using namespace Qore;
...
//    WriteIncludes(this).acceptUI(ui);
    ...
    Qore::WriteDeclaration(this).acceptUI(ui);

    if (option().execCode) {
        QString qualifiedClassName = ui->elementClass() + option().postfix;
		QString className(qualifiedClassName);
		QString parentClass(ui->elementWidget()->attributeClass());
		out << "# This is self-testing class and additional stuff for automatic run of this script" << endl;
        out << "%requires qt-gui" << endl;
		out << "%exec-class QUIC_app__"<< endl << endl;
		out << "class QUIC_app__ inherits QApplication {" << endl;
		out << "    constructor() : QApplication()" << endl;
		out << "	{" << endl;
		out << "		my $w = new " << parentClass << "();" << endl;
		out << "		my $u = new Ui::" << className << "();" << endl;
		out << "		$u.setupUi($w);" << endl;
		out << "		$w.show();" << endl;
		out << "		$.exec();" << endl;
		out << "	}" << endl;
		out << "}" << endl << endl;
