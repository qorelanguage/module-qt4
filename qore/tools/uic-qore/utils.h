...
//#include <QtDebug>
inline QString toQoreIdentifier(const QString &id)
{
//    QString result(id);
...
//    return result;
	...
	identifier.prepend("$.");
//	qDebug() << "toQoreIdentifier" << identifier << id;
	return identifier;
}

inline QString toQoreParent(const QString & id, const QString & mainWidget)
{
	// HACK to handle qore variable scope. Main widget is local. The others are class members.
	if (id.isEmpty())
		return id;
	QString parentWidget(id);
